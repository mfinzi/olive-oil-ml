...
#     pos_scores = (Q.transpose((1,0,2))@P).transpose((1,0,2)) if P is not None else 0
#     weighting = torch.softmax((scores+pos_scores)/np.sqrt(d),axis=-1)
#     weighted_values = weighting@V
#     return weighted_values
...
#         super().__init__()
#         self.heads = nn.ModuleList([SelfAttentionHead(hidden_dim,hidden_dim//num_heads) for _ in range(num_heads)])
#         self.WO = nn.Linear(hidden_dim,hidden_dim)
#         #self.positionalEmbedding = nn.Embedding(2*self.k+1,hidden_dim//num_heads) if rel_pos else None

#     def forward(self,X,pos_scores=None):
#         """Expects X shape (bs,n,hidden_dim)"""
#         bs,n,hd = X.shape
#         if pos_scores is None: pos_scores = [None]*len(self.heads)
#         return self.WO(torch.cat(*[head(X,pos_score) for head, pos_score in zip(self.heads,pos_scores)],dim=-1))
